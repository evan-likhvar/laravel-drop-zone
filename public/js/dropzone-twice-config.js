...
Dropzone.options.intro1Dropzone = {
    ...
    init: function () {
        ...
        $.get('/twice/'+id+'/intro1/get-preview', function (data) {
...
Dropzone.options.intro2Dropzone = {
    //uploadMultiple: true,
    parallelUploads: 1,
    maxFilesize: 16,
    previewTemplate: document.querySelector('#preview').innerHTML,
    ...
    init: function () {
        ...
        $.get('/twice/'+id+'/intro2/get-preview', function (data) {
            let file = {name: data.name, size: data.size};
            myDropzone.options.addedfile.call(myDropzone, file);
            myDropzone.options.thumbnail.call(myDropzone, file, data.link);
            myDropzone.emit("complete", file);
        ...
        this.on("removedfile", function (file) {
            $.post({
                url: '/twice/delete',
                data: {id: id, type: 'intro2', _token: $('[name="_token"]').val()},
                dataType: 'json',
                success: function (data) {
                    $("#response-message").text(data.message);
                    console.log(data.message);
